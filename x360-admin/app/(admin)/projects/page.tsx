...
import {
  ...
  Clock, Users, AlertTriangle, CheckCircle2, Zap,
  ChevronLeft, ChevronRight, Building2, Tag, Pencil,
} from "lucide-react";
import { useStore } from "@/lib/store";
import { useToast } from "@/components/ui/Toast";
import AddProjectModal from "@/components/projects/AddProjectModal";
import type { Project, ProjectStage, Priority } from "@/lib/types";

// ── Stage config ──────────────────────────────────────────────────────────────
const KANBAN_STAGES: ProjectStage[] = [
  ...
  return (
    <div className="glass rounded-xl border border-white/[0.05] hover:border-white/[0.1] transition-all group select-none">
      ...
      <div onClick={onClick} className="px-3 pb-3 cursor-pointer space-y-2">
        ...
        <div>
          ...
          <div className="h-1 rounded-full bg-white/[0.06]">
            <div className={`h-full rounded-full transition-all ${project.progress >= 100 ? "bg-green-400" : project.progress >= 75 ? "bg-amber-400" : "bg-blue-400"}`} style={{ width: `${project.progress}%` }} />
          </div>
        </div>
      ...
      <div className="flex items-center gap-1 px-2 pb-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={e => { e.stopPropagation(); onMoveCard(project, "prev"); }} disabled={!canMovePrev}
        ...
        <button onClick={e => { e.stopPropagation(); onMoveCard(project, "next"); }} disabled={!canMoveNext}
          className="flex items-center justify-center w-6 h-6 rounded bg-white/[0.06] hover:bg-white/[0.12] disabled:opacity-20 transition-colors">
...
function DraggableProjectCard({ project, onClick, onMoveCard, canMovePrev, canMoveNext }: {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Translate.toString(transform) }}
      ...
      <ProjectCard project={project} onClick={onClick} listeners={listeners}
...
function KanbanColumn({ stage, displayLabel, projects, onProjectClick, onAddProject, onMoveCard, onRenameStage, isActive, stageIndex, totalStages }: {
  stage: ProjectStage; displayLabel: string; projects: Project[]; onProjectClick: (p: Project) => void;
  onAddProject: () => void; onMoveCard: (p: Project, dir: "prev" | "next") => void;
  onRenameStage: (stage: string, label: string) => void;
  isActive: boolean; stageIndex: number; totalStages: number;
}) {
  ...
  function commit() { const t = draft.trim(); onRenameStage(stage, t || stage); setEditing(false); }
  ...
  return (
    <div ref={setNodeRef}
      ...
      style={{ width: "220px", scrollSnapAlign: "start", background: isOver ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.02)", borderColor: isOver ? "rgba(255,255,255,0.12)" : undefined }}>
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-white/[0.05] flex items-center gap-2 group/header">
        ...
        {editing ? (
          <input ref={inputRef} value={draft} onChange={e => setDraft(e.target.value)} onBlur={commit} onKeyDown={onKey}
            className="flex-1 min-w-0 bg-white/[0.06] border border-white/[0.15] rounded px-1.5 py-0.5 text-[9px] font-700 text-white/85 uppercase tracking-[0.10em] outline-none focus:border-white/30 font-display" />
        ) : (
          <button onClick={startEdit} title="Click to rename" className="flex-1 min-w-0 flex items-center gap-1.5 text-left group/label">
            <p className="text-[9px] font-700 text-white/70 uppercase tracking-[0.1em] truncate font-display group-hover/label:text-white/90 transition-colors">{displayLabel}</p>
      ...
      <div className="flex-1 p-2 space-y-2 overflow-y-auto min-h-[100px]">
        {projects.map(p => (
          <DraggableProjectCard key={p.id} project={p} onClick={() => onProjectClick(p)}
            onMoveCard={onMoveCard} canMovePrev={stageIndex > 0} canMoveNext={stageIndex < totalStages - 1} />
        ))}
        {projects.length === 0 && (
      ...
      <div className="p-2 border-t border-white/[0.04]">
        <button onClick={onAddProject} className="w-full flex items-center justify-center gap-1 py-1.5 text-[9px] text-white/28 hover:text-white/55 hover:bg-white/[0.03] rounded-lg transition-colors">
...
function ProjectDetailDrawer({ project, onClose }: { project: Project | null; onClose: () => void }) {
  ...
  const { toggleProjectTask } = useStore();
  ...
  return (
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div className="w-[480px] bg-[#0a0a0a] border-l border-white/[0.06] flex flex-col h-full overflow-hidden shadow-[−20px_0_60px_rgba(0,0,0,0.6)]" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="p-5 border-b border-white/[0.05]">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <h2 className="text-[15px] font-700 text-white/90 leading-tight">{project.name}</h2>
              <p className="text-[11px] text-white/40 mt-1 flex items-center gap-1"><Building2 className="w-3 h-3" />{project.client} · {project.type}</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={i.label} className="glass-sm rounded-lg p-3">
                    <p className="text-[9px] text-white/30 uppercase tracking-wide">{i.label}</p>
                    <p className="text-[12px] font-600 text-white/75 mt-0.5">{i.value}</p>
                  </div>
                ))}
              </div>
              <div>
            ...
            <div className="space-y-2">
              {project.tasks.length === 0 ? (
                ...
                <div key={t.id} className="flex items-center gap-3 p-2.5 glass-sm rounded-lg">
                  <button onClick={() => toggleProjectTask(project.id, t.id)}
          ...
          {detailTab === 3 && (
            <div className="space-y-3">
              {[
                { label: "Health Assessment", text: project.health === "Healthy" ? "Project is on track. No major risks detected." : project.health === "At Risk" ? "⚠️ Deadline approaching with current pace. Consider adding resources." : project.health === "Critical" ? "🚨 Critical — immediate intervention required to meet deadline." : "Project is delayed. Reassess scope and timeline with client." },
                { label: "Completion Forecast", text: `At current ${project.progress}% progress rate, project will ${project.daysRemaining > 0 ? `complete within ${project.daysRemaining} days` : "likely miss the deadline"}.` },
                { label: "Value at Risk", text: `SAR ${(project.value/1000000).toFixed(1)}M contract. ${project.health === "Healthy" ? "No financial risk detected." : "Late delivery may trigger penalty clauses."}` },
              ].map(item => (
...
export default function ProjectsPage() {
  const { projects, moveProject, projectStageLabels, renameProjectStage } = useStore();
  ...
  const openAdd = (stage: ProjectStage = "Planning") => {
  ...
  function handleMoveCard(project: Project, direction: "prev" | "next") {
    ...
    toast(`${project.name} → ${targetStage}`);
  }
  ...
  return (
    <div className="space-y-4 max-w-[1600px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          { label: "Total Projects",  value: total,     color: "text-white/85"  },
          { label: "Completed",       value: completed, color: "text-white/85"  },
          { label: "At Risk",         value: atRisk,    color: "text-white/85"  },
          { label: "Critical",        value: critical,  color: "text-white/85"  },
          { label: "Portfolio Value", value: `SAR ${(totalValue/1000000).toFixed(1)}M`, color: "text-white/85" },
        ].map(s => (
          <div key={s.label} className="glass p-3.5 flex items-center gap-2.5">
            <div>
              <p className="text-[10px] text-white/35 uppercase tracking-wide">{s.label}</p>
              <p className={`text-[20px] font-700 font-display mt-0.5 ${s.color}`}>{s.value}</p>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 min-h-[60vh]" style={{ scrollSnapType: "x mandatory" }}>
            {KANBAN_STAGES.map((stage, idx) => (
              <KanbanColumn
                ...
                displayLabel={projectStageLabels[stage] ?? stage}
