...
import {
  ...
  Paperclip, Mic, Smile, ArrowLeft, Bell,
} from "lucide-react";
...
function ChatBg({ dark = "#0b141a" }: { dark?: string }) {
  return (
    <div className="absolute inset-0" style={{ background: dark, backgroundImage: WA_PATTERN }} />
  );
}

type AttachmentProp = { label: string; ext: string };
type BubbleProps = {
  me: boolean; text: string; time: string; accent: string;
  attachment?: AttachmentProp; delay?: number;
};
function Bubble({ me, text, time, accent, attachment, delay = 0 }: BubbleProps) {
  return (
    ...
    >
      ...
      >
        {attachment && (
          <div className="flex items-center gap-1.5 rounded-xl mb-1.5 px-2 py-1.5"
            ...
            <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 text-[10px]"
              style={{ background: `${accent}28` }}>📄</div>
            <div className="min-w-0">
              <div className="text-[7.5px] font-semibold truncate" style={{ color: "rgba(255,255,255,0.75)" }}>{attachment.label}</div>
...
function TypingDots({ accent }: { accent: string }) {
  return (
    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex justify-start">
      <div className="rounded-2xl rounded-tl-sm px-3 py-2 flex gap-1 items-center"
        style={{ background: "rgba(31,44,51,0.96)", boxShadow: "0 1px 3px rgba(0,0,0,0.45)" }}>
        {[0, 1, 2].map(i => (
          <motion.div key={i} className="w-[4px] h-[4px] rounded-full"
...
function DateDivider({ label }: { label: string }) {
  return (
    <div className="flex justify-center py-0.5">
      <div className="text-[6.5px] px-2.5 py-0.5 rounded-full"
        style={{ background: "rgba(11,20,26,0.72)", color: "rgba(255,255,255,0.32)" }}>{label}</div>
    </div>
  );
}

function WAChatHeader({ avatar, name, status, accent }: { avatar: ReactNode; name: string; status: string; accent: string }) {
  return (
    <div className="flex items-center gap-2 px-2.5 py-2 flex-shrink-0"
      ...
      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-[11px]"
        style={{ background: `${accent}28`, border: `1.5px solid ${accent}45` }}>{avatar}</div>
      <div className="flex-1 min-w-0">
        <div className="text-[9.5px] font-semibold text-white leading-tight">{name}</div>
        <div className="flex items-center gap-1">
          <motion.div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: "#25D366" }}
...
function WAChatInput({ placeholder, accent }: { placeholder: string; accent: string }) {
  return (
    <div className="flex items-center gap-1.5 px-2 py-1.5 flex-shrink-0"
      ...
      <div className="flex-1 flex items-center gap-1.5 rounded-full px-2.5 py-1.5"
...
function BillingScreen({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const iv = window.setInterval(() => setScan(n => { if (n >= 100) { clearInterval(iv); return 100; } return n + 3.2; }), 28);
    return () => clearInterval(iv);
  }, [step]);
    ...
    <div className="flex flex-col h-full relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex-1 px-2 py-2 space-y-1.5 overflow-hidden">
          <DateDivider label="Today" />
          {step >= 1 && (
            <Bubble me={true} text="Process this invoice" time="10:15" accent={accent}
              attachment={{ label: "NovaTech_Invoice_1047.pdf", ext: "PDF Document · 284 KB" }} />
          )}
          {step >= 2 && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex justify-start">
              <div className="max-w-[83%] rounded-2xl rounded-tl-sm px-2.5 py-1.5"
                style={{ background: "rgba(31,44,51,0.96)", boxShadow: "0 1px 3px rgba(0,0,0,0.45)" }}>
                <div className="flex items-center gap-1.5 mb-1.5">
                  <span className="text-[9px]">🔍</span>
                  <span className="text-[8px]" style={{ color: "rgba(255,255,255,0.7)" }}>{scan < 100 ? "Scanning invoice…" : "Scan complete"}</span>
                </div>
                <div className="space-y-0.5 mb-1.5">
                  {[["Vendor", "NovaTech Arabia LLC"], ["Invoice", "#INV-2024-1047"], ["Amount", "SAR 8,400.00"], ["Due", "15 December 2024"]].map(([k, v], i) => (
                    <motion.div key={k} className="flex gap-2 text-[7px]" animate={{ opacity: scan > (i + 1) * 22 ? 1 : 0.1 }}>
                ...
                <div className="h-0.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
                  <motion.div className="h-full rounded-full" style={{ width: `${scan}%`, background: `linear-gradient(90deg, ${accent}80, ${accent})` }} />
                </div>
          ...
          {step >= 3 && <Bubble me={false} text={"✅ Invoice processed\n• Category: Professional Services\n• VAT 15%: SAR 1,260.00\n• Status: Approved ✓"} time="10:15" accent={accent} />}
          {step >= 4 && <Bubble me={true} text="Set payment reminder for Dec 10" time="10:16" accent={accent} />}
          {step >= 5 && <Bubble me={false} text="📅 Reminder set for Dec 10. Finance team notified." time="10:16" accent={accent} />}
          <AnimatePresence>{showTyping && <TypingDots accent={accent} />}</AnimatePresence>
        </div>
        ...
        <div className="flex-1 px-2 py-2 space-y-1.5 overflow-hidden">
          ...
          {msgs.slice(0, count).map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28 }}
              ...
              <div className={`max-w-[83%] rounded-2xl px-2.5 py-1.5 text-[8px] leading-relaxed ${m.me ? "rounded-tr-sm" : "rounded-tl-sm"}`}
                style={{ background: m.me ? "rgba(255,255,255,0.08)" : "rgba(245,158,11,0.1)", border: m.me ? "none" : "1px solid rgba(245,158,11,0.18)", color: "rgba(255,255,255,0.85)", boxShadow: "0 1px 3px rgba(0,0,0,0.45)", whiteSpace: "pre-line" }}>
                {m.text}
                <div className="flex items-center gap-1 justify-end mt-0.5">
    ...
    <div className="flex flex-col h-full relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex items-center gap-3 px-3 py-1.5 flex-shrink-0"
          ...
          {[{ label: "Leads", val: tick > 0 ? "48" : "47", hi: tick > 0 }, { label: "Bookings", val: "23", hi: false }, { label: "Revenue", val: "12k", hi: false }].map(m => (
            <motion.div key={m.label} animate={{ color: m.hi ? accent : "rgba(255,255,255,0.55)" }} className="flex items-center gap-1">
              <span className="text-[9px] font-bold">{m.val}</span>
              <span className="text-[6.5px]" style={{ color: "rgba(255,255,255,0.28)" }}>{m.label}</span>
              {m.hi && <motion.span className="text-[6px] font-semibold" style={{ color: "#4ade80" }} initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }}>+1</motion.span>}
            </motion.div>
          ))}
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex justify-start">
              <div className="max-w-[83%] rounded-2xl rounded-tl-sm px-2.5 py-1.5 text-[8px] leading-relaxed"
                ...
                <div className="flex items-center gap-1 mb-0.5">
                  ...
                  <span className="text-[7px] font-semibold" style={{ color: accent }}>New Inquiry</span>
                </div>
                <div>Ahmad Al-Rashidi — table for 4, tomorrow 8 PM</div>
                <div className="flex justify-end mt-0.5"><span className="text-[6.5px]" style={{ color: "rgba(255,255,255,0.32)" }}>19:30</span></div>
              </div>
            </motion.div>
          )}
          {step >= 2 && <Bubble me={false} text="🤖 Auto-replying with availability & pricing…" time="19:30" accent={accent} />}
          {step >= 3 && <Bubble me={false} text={"✅ Reservation confirmed\n• Name: Ahmad Al-Rashidi\n• Table #7 · 4 guests\n• Tomorrow 8:00 PM\n• Deposit: SAR 250 collected"} time="19:31" accent={accent} />}
          {step >= 4 && <Bubble me={true} text="Send him the confirmation message" time="19:31" accent={accent} />}
          {step >= 5 && <Bubble me={false} text="💬 Confirmation sent to Ahmad. CRM updated. Lead marked as converted." time="19:31" accent={accent} />}
        </div>
...
function PhoneFrame({ active }: { active: number }) {
  ...
  return (
    <div className="relative">
      <motion.div className="absolute -inset-10 rounded-[56px] pointer-events-none"
        animate={{ opacity: [0.35, 0.6, 0.35] }} transition={{ duration: 4.5, repeat: Infinity }}
          ...
          <div className="absolute inset-[3px] rounded-[42px] overflow-hidden" style={{ background: "#000000" }}>
            ...
            <div className="overflow-hidden" style={{ position: "absolute", inset: 0, top: 52 }}>
              <AnimatePresence mode="wait">
                <motion.div key={active} initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: -18, scale: 0.97 }} transition={{ duration: 0.55, ease }}
            ...
            <div className="absolute inset-0 pointer-events-none rounded-[42px] overflow-hidden" style={{ zIndex: 5 }}>
              ...
              <motion.div
                ...
                animate={{ left: ["-55%", "155%"] }} transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut", repeatDelay: 7 }} />
            </div>
...
function FloatCard({ text, icon, pos, color }: { text: string; icon: string; pos: FloatPos; color: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[8.5px] font-medium" style={{ color: "rgba(255,255,255,0.58)" }}>{text}</span>
...
type SectionData = {
  ...
  href:   string;
  ctaHref?:    string; ctaLabel?:   string; ctaLabelAr?: string;
...
const SECTIONS: SectionData[] = [
  {
    ...
    sub: "Speak naturally via WhatsApp. Your personal AI handles reminders, emails, schedules, and tasks — before you even ask.",
    subAr: "تحدّث بشكل طبيعي عبر واتساب. يتولى وكيلك الشخصي التذكيرات والجدولة والمهام — قبل أن تطلب.",
    href: "/contact",
    cards: [
  ...
  },
  {
    ...
    sub: "Snap a photo, send a message. Your billing agent scans, categorises, and processes invoices with surgical precision.",
    subAr: "التقط صورة أو أرسل رسالة. يقوم وكيل الفواتير بالمسح والتصنيف والمعالجة بدقة متناهية.",
    href: "/contact",
    cards: [
  ...
  },
  {
    ...
    sub: "From Iqama renewals to labour law — ask in Arabic or English. Get expert, compliant answers instantly.",
    subAr: "من تجديد الإقامة إلى نظام العمل — اسأل بالعربية أو الإنجليزية واحصل على إجابات فورية وملتزمة.",
    href: "/contact",
    cards: [
  ...
  },
  {
    ...
    sub: "Automate reservations, capture leads, and follow up on deals — all through WhatsApp. While you sleep.",
    subAr: "أتمتة الحجوزات وجذب العملاء ومتابعة الصفقات — كل ذلك عبر واتساب. حتى وأنت نائم.",
    href: "/contact",
  ...
  return (
    <div className="hidden lg:flex flex-col gap-1.5">
      {SECTIONS.map((s, i) => {
        ...
        return (
          <motion.div key={i} onClick={() => onSelect(i)}
            ...
            <div className="overflow-hidden">
              <div className="text-[11px] font-semibold whitespace-nowrap" style={{ color: isActive ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.22)" }}>
...
function PhoneShowcaseMobile() {
  ...
  useEffect(() => {
    ...
    const onMove  = (e: TouchEvent) => {
      ...
      if ((dir > 0 && cur < SECTIONS.length - 1) || (dir < 0 && cur > 0)) e.preventDefault();
    };
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden" style={{ height: "100vh" }}>
      ...
      <div className="absolute left-0 top-0 bottom-0 z-10 flex flex-col items-center justify-start gap-3"
        ...
        {SECTIONS.map((sec, i) => {
          ...
          return (
            <motion.button key={i} onClick={() => setActive(i)}
              ...
              animate={{ background: isActive ? `${sec.color}20` : "rgba(255,255,255,0.04)", borderColor: isActive ? `${sec.color}50` : "rgba(255,255,255,0.08)" }}
              whileTap={{ scale: 0.92 }} title={isAr ? sec.badgeAr : sec.badge}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-start" style={{ paddingTop: 64, paddingBottom: 20, gap: 0 }}>
        <AnimatePresence mode="wait">
          <motion.div key={active} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.38, ease }}
            style={{ width: PW, height: PH, flexShrink: 0, position: "relative", overflow: "visible" }}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={`text-${active}`} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            transition={{ duration: 0.35, ease }}
            className="flex flex-col items-center text-center px-8" style={{ gap: 10, marginTop: 16 }}>
            ...
            <h3 className="text-xl font-thin leading-tight whitespace-pre-line" style={{ letterSpacing: "0.1em" }}>{isAr ? s.titleAr : s.title}</h3>
            <div className="h-px w-20" style={{ background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.28), transparent)" }} />
            <p className="text-white/40 text-[13px] leading-relaxed">{isAr ? s.subAr : s.sub}</p>
            <div className="flex items-center gap-3 pt-1">
              <Link href={s.ctaHref ?? "/contact"}>
    ...
    <div ref={containerRef} className="relative w-full h-full overflow-hidden flex items-center justify-center">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid grid-cols-[1fr_300px_1fr] gap-10 items-center">
          ...
          <div className={`flex flex-col ${isAr ? "items-end text-right" : "items-start text-left"}`}>
            <AnimatePresence mode="wait">
              <motion.div key={active}
                ...
                <h3 className="text-[2.6rem] font-thin leading-tight whitespace-pre-line" style={{ letterSpacing: "0.1em" }}>{isAr ? s.titleAr : s.title}</h3>
                ...
                <p className="text-white/40 text-base leading-relaxed max-w-sm">{isAr ? s.subAr : s.sub}</p>
          ...
          {/* ── Right: Side nav ── */}
          <div>
            <SideNav active={active} isAr={isAr} onSelect={(i) => {
              activeRef.current = i;
