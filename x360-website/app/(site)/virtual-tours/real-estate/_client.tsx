...
import {
  Home, Building2, Building, Warehouse, HardHat,
  ArrowRight,
} from "lucide-react";
...
const CATEGORIES = [
  { icon: Home,      title: "Villa & Apartment",    titleAr: "الفيلات والشقق",          href: "/virtual-tours/real-estate/villa-apartment" },
  { icon: Building2, title: "Co-working Spaces",    titleAr: "مساحات العمل المشترك",   href: "/virtual-tours/real-estate/coworking" },
  { icon: Warehouse, title: "Industrial",            titleAr: "الصناعي",                href: "/virtual-tours/real-estate/industrial" },
  { icon: HardHat,   title: "Construction Sites",   titleAr: "مواقع البناء",           href: "/virtual-tours/real-estate/construction" },
  { icon: Building,  title: "Compound",             titleAr: "المجمع السكني",           href: "/virtual-tours/real-estate/compound" },
];

export default function RealEstateCategoriesClient() {
  const { isAr } = useLang();
  ...
  return (
    <>
      ...
      >
        {/* ambient glow */}
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 40%, rgba(107,163,214,0.11) 0%, transparent 65%)" }} />
        {/* grid lines */}
          ...
          <div className="flex flex-col items-center text-center gap-1.5 sm:gap-3">
            ...
            <FadeUp delay={0.26}>
              <p style={{ fontSize: "clamp(0.72rem, 1.3vw, 0.92rem)", color: "rgba(255,255,255,0.42)" }} className="text-center max-w-xs sm:max-w-none hidden sm:block">
                {t("Immersive 360° virtual tours tailored for every real estate segment.", "جولات افتراضية 360° مصمّمة لكل قطاع عقاري.")}
              </p>
            </FadeUp>
          </div>

          {/* cards row */}
          <div className="grid w-full gap-2 sm:gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
            {CATEGORIES.map((cat, i) => {
              const Icon = cat.icon;
              const isLast = i === CATEGORIES.length - 1;
              return (
                <FadeUp key={i} delay={0.32 + i * 0.07} className={isLast ? "col-span-2 sm:col-span-1" : ""}>
                  <Link href={cat.href}>
                    <motion.div
                      className={`relative rounded-xl sm:rounded-2xl flex flex-col items-center justify-center gap-2 sm:gap-3 cursor-pointer overflow-hidden h-[150px] sm:h-[300px]${isLast ? " max-w-[calc(50%-4px)] sm:max-w-none mx-auto" : ""}`}
                      style={{
