...
import {
  Crown, Plane, Anchor, Landmark, Store,
  ArrowRight,
} from "lucide-react";
...
const CATEGORIES = [
  { icon: Crown,    title: "Palaces",         titleAr: "القصور",                        href: "/virtual-tours/luxury-private/palaces" },
  { icon: Plane,    title: "Aircrafts",       titleAr: "الطائرات الخاصة",              href: "/virtual-tours/luxury-private/aircrafts" },
  { icon: Anchor,   title: "Yachts",          titleAr: "اليخوت",                        href: "/virtual-tours/luxury-private/yachts" },
  { icon: Landmark, title: "Private Estate",  titleAr: "المزارع والمنتجعات الخاصة",    href: "/virtual-tours/luxury-private/private-estate" },
  { icon: Store,    title: "Showrooms",       titleAr: "صالات العرض",                  href: "/virtual-tours/luxury-private/showrooms" },
];

export default function LuxuryPrivateCategoriesClient() {
  const { isAr } = useLang();
          ...
          <div className="flex flex-col items-center text-center gap-1.5 sm:gap-3">
            ...
            <FadeUp delay={0.26}>
              <p style={{ fontSize: "clamp(0.72rem, 1.3vw, 0.92rem)", color: "rgba(255,255,255,0.42)" }} className="text-center max-w-xs sm:max-w-none hidden sm:block">
                {t("Immersive 360° virtual tours tailored for every luxury segment.", "جولات افتراضية 360° مصمّمة لكل قطاع فاخر.")}
              </p>
            </FadeUp>
          </div>
            ...
            {CATEGORIES.map((cat, i) => {
              ...
              return (
                <FadeUp key={i} delay={0.32 + i * 0.07} className={isLast ? "col-span-2 sm:col-span-1" : ""}>
                  <Link href={cat.href}>
                    <motion.div
                      className={`relative rounded-xl sm:rounded-2xl flex flex-col items-center justify-center gap-2 sm:gap-3 cursor-pointer overflow-hidden h-[130px] sm:h-[200px]${isLast ? " max-w-[calc(50%-4px)] sm:max-w-none mx-auto" : ""}`}
                      style={{
                    ...
                    >
                      ...
                      {/* click here — hidden on mobile to keep cards compact */}
          ...
          <FadeUp delay={0.72} className="mt-1 sm:mt-4 flex flex-col items-center gap-2 sm:gap-3">
            ...
            <Link href="/services">
              ...
              >
                ...
                {[
                  t("AI Luxury Concierge", "المنسّق الفاخر بالذكاء الاصطناعي"),
                  t("Luxury Website Development", "تطوير مواقع فاخرة"),
                ].map((label, i, arr) => (
