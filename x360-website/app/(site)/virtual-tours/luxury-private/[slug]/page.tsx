...
import { getLuxuryCategory, getAllLuxurySlugs } from "@/data/luxury-categories";
import LuxuryCategoryClient from "./_client";
...
export async function generateStaticParams() {
  return getAllLuxurySlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const data = getLuxuryCategory(slug);
  if (!data) return {};
  const url = `https://www.x-360.ai/virtual-tours/luxury-private/${slug}`;
  return {
...
export default async function LuxuryCategoryPage({ params }: Props) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 4, name: data.name, item: `https://www.x-360.ai/virtual-tours/luxury-private/${slug}` },
    ],
  };
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} />
      <LuxuryCategoryClient data={data} />
    </>
  );
}
