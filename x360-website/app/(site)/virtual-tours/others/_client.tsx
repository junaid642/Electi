...
import {
  GraduationCap, Landmark, CalendarDays, Building, Cross,
  ArrowRight,
} from "lucide-react";
...
const CATEGORIES = [
  { icon: GraduationCap, title: "Schools & Universities", titleAr: "المدارس والجامعات",  href: "/virtual-tours/others/schools" },
  { icon: Landmark,      title: "Government Buildings",   titleAr: "المباني الحكومية",    href: "/virtual-tours/others/government" },
  { icon: CalendarDays,  title: "Event Spaces",           titleAr: "قاعات الفعاليات",     href: "/virtual-tours/others/event-halls" },
  { icon: Building,      title: "Medical Centers",        titleAr: "المراكز الطبية",      href: "/virtual-tours/others/medical" },
  { icon: Cross,         title: "Museums",                titleAr: "المتاحف",             href: "/virtual-tours/others/museums" },
];

export default function OthersCategoriesClient() {
  const { isAr } = useLang();
  ...
  return (
    <>
      ...
      >
        {/* ambient glow */}
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 40%, rgba(100,180,120,0.09) 0%, transparent 65%)" }} />
        {/* grid lines */}
          ...
          <div className="flex flex-col items-center text-center gap-1.5 sm:gap-3">
            ...
            <FadeUp delay={0.26}>
              <p style={{ fontSize: "clamp(0.72rem, 1.3vw, 0.92rem)", color: "rgba(255,255,255,0.42)" }} className="text-center max-w-xs sm:max-w-none hidden sm:block">
                {t("Immersive 360° virtual tours tailored for every type of venue.", "جولات افتراضية 360° مصمّمة لكل نوع من المنشآت.")}
              </p>
            </FadeUp>
          </div>
