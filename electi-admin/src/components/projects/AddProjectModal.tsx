...
import { z } from "zod";
import { Building2, FileText, Tag, Users2, ChevronRight, ChevronLeft, Check } from "lucide-react";
import Modal from "@/components/ui/Modal";
import { useProjects } from "@/lib/project-store";
import { toast } from "@/hooks/use-toast";
import type { ProjectStage, Priority, ProjectHealth } from "@/lib/project-types";
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Pipeline Stage *" error={errors.stage?.message}>
                  <select {...register("stage")} className={selectCls}>
                    {KANBAN_STAGES.map(s => <option key={s} value={s} className="bg-[#0a0a0a]">{s}</option>)}
                  </select>
                </Field>
